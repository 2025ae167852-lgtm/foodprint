...
const { uploadPDF, resolveFilenames } = require('../config/cloudinary/file-upload');
...
try {
  ...
  console.warn('Warning: Could not load database config in produce routes:', err.message);
  sequelize = null;
}
...
if (sequelize) {
  try {
    ...
    console.warn('Warning: Could not initialize models in produce routes:', err.message);
  }
}

// Lazy load PDF service to avoid loading errors when module is not installed
var pdfService = null;
var producepricepdf = null;
try {
  pdfService = require('../config/pdf/pdf-service');
  producepricepdf = require('../config/pdf/producepricepdf');
} catch (err) {
      ...
      .then(rows => {
        ...
        pdfService.buildPDF(
          ...
          (err, data) => {
            ...
            //Cloudinary upload
            uploadPDF(result, filenames.filename, 'foodprint/produce/pdfs')
              .then(uploadResult => {
