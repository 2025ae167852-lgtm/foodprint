...
try {
  // Check if DATABASE_URL is set before requiring db config
  if (process.env.DATABASE_URL) {
    sequelize = require('../config/db/db_sequelise');
  }
} catch (err) {
  console.warn('Warning: Could not load database config in harvest routes:', err.message);
  sequelize = null;
}
...
if (sequelize) {
  try {
    ...
    console.warn('Warning: Could not initialize models in harvest routes:', err.message);
  }
}

const { uploadFile, uploadPDF, resolveFilenames } = require('../config/cloudinary/file-upload');
const { getMimeType } = require('../utils/image_mimetypes');

// Lazy load PDF service to avoid loading errors when module is not installed
var pdfService = null;
var harvestpdf = null;
try {
  pdfService = require('../config/pdf/pdf-service');
  harvestpdf = require('../config/pdf/harvestpdf');
} catch (err) {
  console.warn('PDF service not available:', err.message);
}

// Lazy load Twilio to avoid errors when not configured
var tw_client = null;
try {
...
router.get('/test', function (req, res) {
  res.send('Harvest route is mounted successfully!');
});
...
router.get(
  ...
  function (req, res, next) {
    console.log('Harvest route handler called');

    // Check if models are available
    if (!models || !models.FoodprintHarvest) {
      req.flash(
        'error',
    ...
    if (!result.isEmpty()) {
      ...
      try {
        fs.readFile(img.path, async function (err, img_datadata) {
          ...
          try {
            const uploadResult = await uploadFile(fileObj, harvest_logid_uuid, 'foodprint/harvest');
            console.log('File uploaded to Cloudinary:', uploadResult.secure_url);

            harvest_entry['harvest_image_url'] = uploadResult.secure_url;
    ...
    try {
      ...
      tw_client
        ...
        .then(media => {
          ...
          const fileObj = {
            buffer: harvest_photoHash,
            mimetype: contentType,
            originalname: `${harvest_logid_uuid}.${ext}`,
          };

          uploadFile(fileObj, harvest_logid_uuid, 'foodprint/harvest')
            .then(uploadResult => {
              console.log('File uploaded to Cloudinary:', uploadResult.secure_url);
              harvest_entry['harvest_image_url'] = uploadResult.secure_url;
            ...
            .catch(error => {
              ...
              // Still create harvest entry without image
              createHarvestEntry(harvest_entry);
            });
        });
    } catch (e) {
          ...
          .then(rows => {
            ...
            ${moment(new Date()).format('YYYY-MM-DD HH:mm:ss')}`,
              ...
              (err, data) => {
                ...
                //Cloudinary upload
                uploadPDF(result, filenames.filename, 'foodprint/harvest/pdfs')
                  .then(uploadResult => {
                    console.log('PDF uploaded to Cloudinary:', uploadResult.secure_url);
                    res
                      .status(200)
                      .send({ message: 'file uploaded', pdf_url: uploadResult.secure_url });
                  })
                  .catch(error => {
                    console.error('Cloudinary upload error:', error);
            ...
            if (rows[i].harvest_photoHash === null) {
              ...
              try {
                ...
                const fileObj = {
                  ...
                  originalname: `${rows[i].harvest_logid}${metadata.ext}`,
                };

                uploadFile(fileObj, rows[i].harvest_logid, 'foodprint/harvest')
                  .then(uploadResult => {
                    console.log('File uploaded to Cloudinary:', uploadResult.secure_url);
                    const harvest_entry = {
                      harvest_image_url: uploadResult.secure_url,
                    };
