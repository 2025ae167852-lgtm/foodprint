...
try {
  ...
  console.warn('Warning: Could not load database config in storage routes:', err.message);
  sequelize = null;
}
...
if (sequelize) {
  try {
    ...
    console.warn('Warning: Could not initialize models in storage routes:', err.message);
  }
}
...
try {
  ...
  storagepdf = require('../config/pdf/storagepdf');
} catch (err) {
          ...
          .then(rows => {
            ...
            ${moment(new Date()).format('YYYY-MM-DD HH:mm:ss')}`,
              ...
              (err, data) => {
                ...
                //Cloudinary upload
                uploadPDF(result, filenames.filename, 'foodprint/storage/pdfs')
                  .then(uploadResult => {
